...
> **Updated:** 2026-07-07 | **Ref:** [RDash](https://rdash.io/) | **Branch:** `main`
...
| **RFQ & Quotes**    | Procurement packages (multi-item, multi-vendor), service-area vendor suggestion, issue/invite (email fan-out) with per-vendor distribution tracking (method / sent-by), vendor portal submit/revise, partial per-item bidding, manual & multi-channel quote entry + evidence metadata (uploader/date/source), quote versioning, comparison matrix (price / lead time / payment terms / rating / prior awards), single + split award, RFQ revisions (supersede), scope-change impact routing, communication timeline, audit trail |
...
> **RFQ Phase 2 — shipped:** partial per-item bidding (§14), quote evidence metadata (§15), per-vendor distribution tracking (§11), comparison decision criteria (§19). Remaining pre-PO polish + the scope-change workflow are tracked in `docs/rfq-boq-rc-completion-plan.md`.
