...
`submitOrUpdateQuote` rejects any quote not covering _every_ RFQ item
...
status already carries submitted/rejected/expired. Model "no bid" as simply _no
`vendor_quote_item` row_ for that rfq_item. Satisfies "some/none" without a new
