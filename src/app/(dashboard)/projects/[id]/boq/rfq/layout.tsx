 ...
 * state. This layout bounces external viewers back to the BOQ scope tab so
 * the procurement surface is unreachable end-to-end.
 */
export default function BoqRfqLayout({
  params,
  children,
}: {
  ...
  useEffect(() => {
    if (!loading && blocked) {
      router.replace(`/projects/${projectId}/boq/${DEFAULT_BOQ_SEGMENT}`);
    }
