 ...
 *
 * Used by both the BOQ payload scrub (server) and the BOQ table/drawer
 * UI gating (client). Keep this as the single source of truth — checking
 * `role === "client"` in one place and `isExternalViewer` in another lets
 * vendor slip through one half of the gate.
 */
